...
def generate_unique_numbers(count, min_keg, max_keg):
    """Генерация набора случайных чисел в заданном количестве и диапазоне"""
    if count > max_keg - min_keg + 1:
        raise ValueError('Введены некоректные параметры.')
    numbers = []
...
class Keg:
    """Класс бочонок, выдает рандомное число"""
    __num = None
...
class Card:
    """Класс карточки"""
    __rows = 3  # количество строк
    __cols = 9  # количество элементов в строке
    __nums_in_row = 5  # количество чисел в сроке
    __data = None  # формируемая карточка
    __emptynum = 0  # пустой элемент
    __crossednum = -1  # зачеркнутый элемент
